...
This document tracks the automation status and reusability of QA tests for the Focus Server and Panda UI project, including analysis of BIT (Built-In Test) usage and automation coverage.

---

## 🎯 Original Content - Brainstorm Meeting (24.03.2025)

### Q&A Analysis of Using Existing BIT in Testing Framework

**Discussion Comments:** (# = discussion comment)

#### Pros:
- Use existing tools - avoid reinventing the wheel

#### Both Pros & Cons:
- Tools developed by QA team will extend product monitoring but at the same time will be a part of product while they should be independent

#### Cons:
- Testing environment should be independent from Dev env and tools
- BIT is a part of product and as such should be also tested - this requires separate implementation.
  - # agreed by all sides
- BIT quality is not proven - need to be proved by QA first
  - # may be true for everything what is not QA proved
- Telegraf and metrics dependent (new layer of QA risk)
  - # agreed by all sides | because telegraf had bugs in the past - it is a big true risk
- New/modified functionalities made by QA may impact production and product itself, decrease stability or low performance
  - # agreed by all sides
- Inter-team impact
  - QA → Dev - any BIT change by QA cause QA is Dev team dependent
  - Dev → QA - any change not consulted and approved by QA team may break test.
  - # agreed by all sides
- BIT checks the data frames with the system and QA wants to check whole flow e.g. dump data to MongoDB, not only check if Collector got data.
  - # agreed by all sides → BIT may offer this in the future but do not offer now and QA need now

### Conclusions:
1. At early beginning use BIT
2. Later implement own tool by QA team
...
2. 100% automation status Unknown (before labels update)
...
- `test_config_validation_nfft_frequency.py`: 7 markers
- `test_mongodb_indexes_and_schema.py`: 5 markers
- `test_historic_playback_additional.py`: 5 markers
- `test_prelaunch_validations.py`: 5 markers
- `test_api_endpoints_high_priority.py`: 4 markers
...
5. **BIT Usage:** Following the conclusions from the brainstorm meeting (24.03.2025), QA team is using BIT at early stages and will implement own tools later
...
5. **BIT Implementation**
   - Continue using BIT at early stages
   - Plan implementation of own QA tools for later stages
...
### Automation Coverage
- **Total Test IDs with Markers:** 162
- **Tests with Test Functions:** 160 (98.8%)
