...
try:
    ...
    print("❌ paramiko is required. Install with: py -m pip install paramiko")
    print("   or: pip install paramiko")
    sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Setup GitHub Actions Runner on Slave Laptop (10.50.0.36)",
...
Examples:
  ...
  # With password:
    py scripts/setup_runner_on_slave_laptop.py --user admin --password mypass
  
  # With SSH key:
    py scripts/setup_runner_on_slave_laptop.py --user admin --key ~/.ssh/id_rsa
  
  # Custom runner name and path:
    py scripts/setup_runner_on_slave_laptop.py --user admin --password mypass --runner-name my-runner --install-path C:\\my-runner
        """
    )
    
    parser.add_argument("--user", "-u", help="SSH username")
    parser.add_argument("--password", "-p", help="SSH password (not recommended - use key or prompt)")
    parser.add_argument("--key", "-k", help="SSH key file path")
    parser.add_argument("--slave-ip", default="10.50.0.36", help="Slave laptop IP address")
    parser.add_argument("--repo-url", default="https://github.com/PrismaPhotonics/panda-backend-api-tests", help="GitHub repository URL")
    parser.add_argument("--runner-name", default="slave-laptop-runner", help="Runner name")
    parser.add_argument("--install-path", help="Installation path (auto-detected based on OS)")
    parser.add_argument("--token", "-t", help="GitHub registration token (if not provided, will prompt)")
    ...
    if args.key:
        ssh_key_file = Path(args.key).expanduser()
    ...
    try:
        ...
        if args.install_path:
            ...
            default_path = "C:\\actions-runner" if os_type == "windows" else "/opt/actions-runner"
