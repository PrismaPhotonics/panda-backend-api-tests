#!/usr/bin/env python3
"""
Copy SSH Public Key to Production Target Host via Jump Host
"""
...
# SSH key path
SSH_KEY_PATH = Path.home() / ".ssh" / "panda_production_key.pub"

def main():
    print("\n" + "=" * 80)
    print("Copying SSH Key to Production Target Host via Jump Host")
    print("=" * 80)
    
    # Step 1: Read public key
    if not SSH_KEY_PATH.exists():
        print(f"[ERROR] Public key not found: {SSH_KEY_PATH}")
        print(f"[INFO] Please generate SSH key first:")
        print(f"  ssh-keygen -t ed25519 -f {SSH_KEY_PATH.parent / 'panda_production_key'} -N '\"\"'")
        return 1
    ...
    try:
        ...
        temp_key_file = "/tmp/panda_production_key.pub"
        ...
        if exit_code == 0:
            ...
            print(f"\n  4. Exit and test:")
            print(f"     ssh -i {SSH_KEY_PATH.parent / 'panda_production_key'} -o ProxyJump={JUMP_USER}@{JUMP_HOST} {TARGET_USER}@{TARGET_HOST} 'hostname'")
